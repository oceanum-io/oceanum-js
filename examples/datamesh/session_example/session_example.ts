...
async function directSessionExample() {
  ...
  try {
    const session = await Session.acquire(connector, { duration: 3600 });
...
async function asyncAwaitSessionExample() {
  ...
  try {
    ...
    const datasource1 = await connector.getDatasource(
      "example-datasource-id-1"
    );
    console.log(`Retrieved datasource 1: ${datasource1.id}`);

    const datasource2 = await connector.getDatasource(
      "example-datasource-id-2"
    );
